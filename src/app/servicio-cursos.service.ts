...
export class ServicioCursosService {
  ...
  cargarCursos():{ nomCurso: string, nomProfesor: String, fechaInicio: Date, fechaFinal: Date, descripcion: string }[] {
    this.cursos = JSON.parse(localStorage.getItem('listaCursos') || '[]');
    return this.cursos;
  }
  nuevoCurso(nomCurso: string, nomProfesor: string, fechaInicio: Date, fechaFinal: Date, descripcion: string){
    let cur = { nomCurso: nomCurso, nomProfesor: nomProfesor, fechaInicio: fechaInicio, fechaFinal: fechaFinal, descripcion: descripcion }
